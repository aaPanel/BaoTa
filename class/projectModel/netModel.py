...
import traceback
import datetime
if not '/www/server/panel/class/' in sys.path: sys.path.insert(0, '/www/server/panel/class/')
...
class main(projectBase):
    ...
    _net_path = '/www/server/net_project'  #
    _log_name = '项目管理'
    _net_pid_path = '/var/tmp/net_project'
    _net_logs = '{}/logs'.format(_net_path)
    _net_logs_path = "/www/wwwlogs/net"
    _net_run_scripts = '{}/scripts'.format(_net_path)
    ...
    def __init__(self):
        try:
            if not os.path.exists(self._net_path):
                os.makedirs(self._net_path, 493)
        except FileExistsError:
            pass

        if not os.path.exists(self._net_pid_path):
            public.ExecShell("mkdir -p /var/tmp/net_project/ && chmod 777 /var/tmp/net_project/")

        if not os.path.exists(self._net_logs_path):
            os.makedirs(self._net_logs_path, 493)

        if not os.path.exists(self._net_run_scripts):
            os.makedirs(self._net_run_scripts, 493)

    # 获取系统所有的用户
    def get_system_user(self, get=None):
        '''
        @name 获取系统所有的用户
        @Author:sww
        @return list
    ...
    # 取当前可用net版本
    def GetNetVersion(self, get=None):
        if os.path.exists('/www/server/dotnet/'):
            entries = os.listdir("/www/server/dotnet/")
            data = [entry for entry in entries if os.path.exists(os.path.join("/www/server/dotnet", entry, 'dotnet'))]
        else:
            data = []
        return data
    ...
    def get_project_find(self, project_name):
        '''
            ...
            @return dict
        '''
        project_info = public.M('sites').where('project_type=? AND name=?', ('net', project_name)).find()
    ...
    def get_net_pids(self, pid):
        ...
        '''
        plugin_name = None
        for pid_name in os.listdir(self._net_pid_path):
            pid_file = '{}/{}'.format(self._net_pid_path, pid_name)
            try:
    ...
    def get_project_pids(self, get=None, pid=None):
        ...
        for i in project_pids:
            net_pids += self.get_project_pids(pid=i)
        if os.path.exists('/proc/{}'.format(pid)):
            project_pids.append(pid)

        all_pids = list(set(project_pids + net_pids))
        if not all_pids:
            all_pids = self.get_net_pids(pid)
    ...
    def get_project_run_state(self, get=None, project_name=None):
        ...
        pid_file = "{}/{}.pid".format(self._net_pid_path, project_name)
    ...
    def get_ssl_end_date(self, project_name):
        ...
        '''
        import data
        return data.data().get_site_ssl_info('net_{}'.format(project_name))
    ...
    def get_project_stat(self, project_info):
        ...
        '''
        project_info['project_config'] = json.loads(project_info['project_config'])
        bind_extranet = project_info['project_config'].get('bind_extranet', 0)
        if isinstance(bind_extranet, str):
            bind_extranet = 1 if bind_extranet.lower() == 'true' else 0
        project_info['project_config']['bind_extranet'] = int(bind_extranet)
    ...
    def exists_nginx_ssl(self, project_name):
        '''
            ...
            @return tuple
        '''
        config_file = "{}/nginx/net_{}.conf".format(public.get_vhost_path(), project_name)
    ...
    def check_port_is_used(self, port, sock=False):
        ...
        project_list = public.M('sites').where('status=? AND project_type=?', (1, 'net')).field(
    ...
    def exists_apache_ssl(self, project_name):
        '''
            ...
            @return bool
        '''
        config_file = "{}/apache/net_{}.conf".format(public.get_vhost_path(), project_name)
        if not os.path.exists(config_file):
            return False, False
    ...
    def set_apache_config(self, project_find):
        ...
        config_file = "{}/apache/net_{}.conf".format(self._vhost_path, project_name)
        template_file = "{}/template/apache/net_http.conf".format(self._vhost_path)
    ...
    #HTTP_TO_HTTPS_END'''
        ...
        config_file = "{}/nginx/net_{}.conf".format(self._vhost_path, project_name)
        template_file = "{}/template/nginx/net_http.conf".format(self._vhost_path)
        ...
        rewrite_file = "{panel_path}/vhost/rewrite/net_{project_name}.conf".format(panel_path=self._panel_path,
                                                                                   project_name=project_name)
    ...
    def bind_extranet(self, get):
        ...
        public.WriteLog(self._log_name, '.net项目{}, 开启外网映射'.format(project_name))
    ...
    def unbind_extranet(self, get):
        ...
        public.M('sites').where("id=?", (project_find['id'],)).setField('project_config',
                                                                        json.dumps(project_find['project_config']))
        public.WriteLog(self._log_name, 'net项目{}, 关闭外网映射'.format(project_name))
    ...
    def stop_project(self, get):
        ...
        pid_file = "{}/{}.pid".format(self._net_pid_path, get.project_name)
    ...
    def start_project(self, get):
        ...
        if not os.path.exists(self._net_pid_path):
            ...
            ret = public.get_mode_and_user("/var/tmp/net_project/")
            if isinstance(ret, dict):
                if ret['mode'] != 777:
                    public.ExecShell("chmod 777 /var/tmp/net_project/")
        project_find = self.get_project_find(get.project_name)
        ...
        if 'project_path' in project_find['project_config']:
            net_path = project_find['project_config']['project_path']
        else:
            net_path = project_find['path']

        if os.path.isfile(net_path):
            net_path = os.path.dirname(net_path)
...
echo $! > {pid_file}'''.format(
        ...
        script_file = "{}/{}.sh".format(self._net_run_scripts, get.project_name)
        # 写入启动脚本
        ...
        public.ExecShell("chown -R {}:{} {}".format(project_find['project_config']['run_user'],
                                                    project_find['project_config']['run_user'], net_path))
        public.set_mode(script_file, 755)
        public.set_own(script_file, project_find['project_config']['run_user'])
        # 执行脚本文件
        env_dict = os.environ.copy() # 没有HOME环境变量执行dotnet会报错
        if "HOME" not in env_dict:
            env_dict["HOME"] = "/root"
        p = public.ExecShell("bash {}".format(script_file), user=project_find['project_config']['run_user'], env=env_dict)
        time.sleep(1)
    ...
    def get_project_list(self, get):
        ...
        if 'search' in get:
            ...
            if type_id is None:
                count = public.M('sites').where('project_type=? AND (name LIKE ? OR ps LIKE ?)', ('net', search, search)).count()
                data = public.get_page(count, int(get.p), int(get.limit), get.callback)
                data['data'] = public.M('sites').where('project_type=? AND (name LIKE ? OR ps LIKE ?)', ('net', search, search)).limit(data['shift'] + ',' + data['row']).order(get.order).select()
            else:
                count = public.M('sites').where('project_type=? AND (name LIKE ? OR ps LIKE ?) AND type_id = ?',
                                                ('net', search, search, type_id)).count()
                data = public.get_page(count, int(get.p), int(get.limit), get.callback)
                data['data'] = public.M('sites').where('project_type=? AND (name LIKE ? OR ps LIKE ?) AND type_id = ?', ('net', search, search, type_id)).limit(data['shift'] + ',' + data['row']).order(get.order).select()
        else:
            if type_id is None:
                count = public.M('sites').where('project_type=?', 'net').count()
                data = public.get_page(count, int(get.p), int(get.limit), get.callback)
                data['data'] = public.M('sites').where('project_type=?', 'net').limit(data['shift'] + ',' + data['row']).order(get.order).select()
            else:
                count = public.M('sites').where('project_type=? AND type_id = ?', ('net', type_id)).count()
                data = public.get_page(count, int(get.p), int(get.limit), get.callback)
                data['data'] = public.M('sites').where('project_type=? AND type_id = ?', ('net', type_id)).limit(data['shift'] + ',' + data['row']).order(get.order).select()
    ...
    def get_project_info(self, get):
        '''
            ...
            @return dict
        '''
        project_info = public.M('sites').where('project_type=? AND name=?', ('net', get.project_name)).find()
    ...
    def create_project(self, get):
        ...
        try:
            ...
            public.set_module_logs("create_dotnet_project", "create")
            ...
            # 项目目录不能包含空白字符
            if re.search('\s', get.project_path):
                return public.return_error('项目目录不能包含空白字符')
            ...
            if "dotnet" == get.project_cmd[:6] and get.dotnet_version:
                get.project_cmd = '/www/server/dotnet/{}/dotnet '.format(get.dotnet_version) + get.project_cmd[6:].strip()
            # 获取可执行文件的的根目录
            pdata = {
                ...
                'project_config': json.dumps(
                    {
                        'ssl_path': '/www/wwwroot/net_node_ssl',
                        ...
                        'port': int(get.port),
                        'log_path': self._net_logs_path,
                        'dotnet_version': get.dotnet_version,
                    }
                ),
            ...
            self.set_config(get.project_name)
            public.WriteLog(self._log_name, '添加net项目{}'.format(get.project_name))
    ...
    def modify_project(self, get):
        ...
        try:
            ...
            if hasattr(get, 'release_firewall'): project_find['release_firewall'] = get.release_firewall
            if hasattr(get, 'ps'): project_find['ps'] = get.ps.strip()
            if hasattr(get, 'project_ps'): project_find['ps'] = get.project_ps.strip()
            pdata = {
            ...
            self.set_config(get.project_name)
            public.WriteLog(self._log_name, '修改net项目{}'.format(get.project_name))
            # 重启项目
    ...
    def remove_project(self, get):
        ...
        if "log_path" not in project_find['project_config']:
            log_file = "{}/{}.log".format(self._net_logs, project_find["name"])
        else:
            log_file = "{}/{}.log".format(project_find['project_config']["log_path"], project_find["name"])
        ...
        remove_sites_service_config(get.project_name, "net_")
        public.WriteLog(self._log_name, '删除net项目{}'.format(get.project_name))
    ...
    def get_project_log(self, get):
        ...
        if "log_path" not in project_info['project_config']:
            log_file = "{}/{}.log".format(self._net_logs, project_info["name"])
        else:
    ...
    def auto_run(self):
        ...
        '''
        # 获取数据库信息
        project_list = public.M('sites').where('project_type=?', ('net',)).field('name,path,project_config').select()
        get = public.dict_obj()
        success_count = 0
        error_count = 0
        for project_find in project_list:
            ...
            if not project_state:
                ...
                if not result['status']:
                    error_count += 1
                    error_msg = '自动启动net项目[' + project_name + ']失败!'
                    public.WriteLog(self._log_name, error_msg)
                else:
                    success_count += 1
                    success_msg = '自动启动net项目[' + project_name + ']成功!'
                    public.WriteLog(self._log_name, success_msg)
        if (success_count + error_count) < 1: return False
        dene_msg = '共需要启动{}个net项目，成功{}个，失败{}个'.format(success_count + error_count, success_count, error_count)
    ...
    def change_log_path(self, get):
        ...
        res = self.stop_project(get)
        res = self.start_project(get)
        public.WriteLog(self._log_name, 'net项目{}, 修改日志路径成功'.format(get.project_name))
    ...
    def del_crontab(self, name):
        ...
        @return 
        """
        cron_name = f'[勿删]net项目[{name}]运行日志切割'
    ...
    def change_cronta(self, name, log_conf):
        ...
        cronInfo = public.M('crontab').where('name=?', (f'[勿删]net项目[{name}]运行日志切割',)).find()
        if not cronInfo:
    ...
    def _update_project(self, project_name, project_info):
        ...
        # 移动日志文件
        # 保存
        target_file = self._net_logs_path + "/" + project_name + ".log"
        if "log_path" in project_info['project_config']:
            return
        log_file = "{}/{}.log".format(self._net_logs, project_name)
        ...
        project_info['project_config']["log_path"] = self._net_logs_path
        pdata = {
            'name': project_name,
            'project_config': json.dumps(project_info['project_config'])
        }
    ...
    def get_project_status(self, project_id):
        # 仅使用在项目停止告警中
        project_info = public.M('sites').where('project_type=? AND id=?', ('net', project_id)).find()
        if not project_info:
...
if __name__ == "__main__":
    m = main()
    print(m.get_system_user())
    print(m.GetNetVersion())
