...
var host_trem = {
  ...
  init: function () {
      ...
      if (_tool_status == 0) {
          ...
          $(".term_box").css("margin-right", "300px");
      ...
      // 添加服务器和常用秘钥
      // $('.term_tootls ul a').click(function () {
      ...
      // 右侧tab点击添加
      $('.term_tootls .btn').click(function () {
          var type = $(this).data('type');
          switch (type) {
              case 'host':
                  that.editor_host_view();
  ...
  */
  reader_host_list: function (callback) {
      ...
      this.get_host_list(function (rdata) {
          bt.each(rdata, function (index, item) {
              html += '\
                  <li data-host="' + item.host + '" data-index="' + index + '">\
                      <i></i>\
                      <span>' + (item.ps == item.host ? item.ps : (item.ps + '【' + item.host + '】')) + '</span>\
                      <span class="tootls">\
