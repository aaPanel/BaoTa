ace.define(
	'ace/mode/nginx_highlight_rules',
			...
			s = function () {
				...
				;(this.$rules = {
					...
					lexical: [
						...
						{ token: 'keyword.operator', regex: '\\B(\\+|\\-|\\*|\\=|!=)\\B' },
						{ token: 'constant.language', regex: '\\b(true|false|on|off|all|any|main|always)\\b' },
						{ token: 'text', regex: '\\s+' },
						{ include: 'variables' },
					],
					variables: [
						{ token: 'variable', regex: '\\$[\\w_]+' },
						{ token: 'variable.language', regex: '\\b(GET|POST|HEAD)\\b' },
					],
				}),
					this.normalizeRules()
			}
