		...
		var r = e('../lib/oop'),
			...
			s = function () {
				var e =
					...
					c = { token: 'constant.language.boolean', regex: '(?:True|False)\\b' },
					h = { token: 'constant.other', regex: 'v[0-9](?:\\.[a-zA-Z0-9*])*\\b' },
					p = { token: s, regex: '[a-zA-Z][\\:a-zA-Z0-9_-]*\\b' },
					d = { token: 'variable.language', regex: '[$@%&][?*!.]?[a-zA-Z0-9_-]+\\b' },
					...
					g = { token: 'constant.language', regex: '\ud835\udc52|\u03c0|\u03c4|\u221e' },
					y = { token: 'string.quoted.single', regex: "['](?:(?:\\\\.)|(?:[^'\\\\]))*?[']" },
					b = { token: 'string.quoted.single', regex: '[<](?:[a-zA-Z0-9 ])*[>]' },
					w = {
						token: 'string.regexp',
						regex:
							'[m|rx]?[/](?:(?:\\[(?:\\\\]|[^\\]])+\\])|(?:\\\\/|[^\\]/]))*[/]\\w*\\s*(?=[).,;]|$)',
				...
				this.$rules = {
					start: [
						...
						{ token: 'comment.block', regex: '#[`|=]\\[.*\\]' },
						{ token: 'comment.doc', regex: '^=(?:begin)\\b', next: 'block_comment' },
						{ token: 'string.unquoted', regex: 'q[x|w]?\\:to/END/;', next: 'qheredoc' },
						{ token: 'string.unquoted', regex: 'qq[x|w]?\\:to/END/;', next: 'qqheredoc' },
						w,
						y,
					...
					block_comment: [
						{ token: 'comment.doc', regex: '^=end +[a-zA-Z_0-9]*', next: 'start' },
			...
			var r = e('../lib/oop'),
				...
				a = function () {
					;(this.HighlightRules = s),
						(this.$outdent = new o()),
						(this.foldingRules = new u({ start: '^=(begin)\\b', end: '^=(end)\\b' })),
