ace.define(
	...
	ace.define(
		'ace/mode/sqlserver_highlight_rules',
		[
			'require',
			'exports',
			'module',
			...
			var r = e('../lib/oop'),
				...
				o = function () {
					...
					var t =
						...
						n =
							'BIGINT|BINARY|BIT|CHAR|CURSOR|DATE|DATETIME|DATETIME2|DATETIMEOFFSET|DECIMAL|FLOAT|HIERARCHYID|IMAGE|INTEGER|INT|MONEY|NCHAR|NTEXT|NUMERIC|NVARCHAR|REAL|SMALLDATETIME|SMALLINT|SMALLMONEY|SQL_VARIANT|TABLE|TEXT|TIME|TIMESTAMP|TINYINT|UNIQUEIDENTIFIER|VARBINARY|VARCHAR|XML',
						r =
					...
					;(s +=
						...
						(s += '|LOOP|HASH|MERGE|REMOTE'),
						(s += '|TRY|CATCH|THROW'),
						(s += '|TYPE'),
						(s = s.split('|')),
						(s = s.filter(function (r, i, s) {
							return (
								e.split('|').indexOf(r) === -1 &&
								t.split('|').indexOf(r) === -1 &&
								n.split('|').indexOf(r) === -1
						...
						start: [
							{
								...
								next: [
									...
									{ token: 'string.end', next: 'start', regex: "'" },
									{ defaultToken: 'string' },
								],
							},
							...
							{ token: o, regex: '@{0,2}[a-zA-Z_$][a-zA-Z0-9_$]*\\b(?!])' },
					...
					for (var f = 0; f < u.length; f++)
						this.$rules.start.unshift({ token: 'set.statement', regex: u[f] })
